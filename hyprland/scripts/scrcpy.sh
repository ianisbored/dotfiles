...
if [ "$device_count" -gt 1 ]; then
    selected_device=$(echo "$devices" | rofi -dmenu -config ~/.config/dotfiles/hyprland/rofi/config.rasi -p "Select Device: ")
else
...
selected_option=$(echo -e "$options" | rofi -dmenu -config ~/.config/dotfiles/hyprland/rofi/config.rasi -p "Select SCRCPY mode: ")
